...
var Component = exports.Component = function () {
	...
	_createClass(Component, [{
		...
		value: function elementCreated(element, parentWrapper) {
			...
			if (this.hasAttribute(element, "onClick")) {
				// Warn about incorrect case for onclick
			}

			if (this.hasAttribute(element, "onclick")) {
				var value = element.getAttributes().get("onclick").getValue();
				value = value + "_" + this._idSuffix;
				element.getAttributes().get("onclick").setValue(value);
				wrapper.getMappedElement().onclick = function (event) {
					events.trigger(value, event);
				};
			}
    ...
    _createClass(EventManager, [{
        ...
        value: function trigger(eventName, event) {
            ...
            if (this._listeners.exists(eventName)) {
                ...
                this._listeners.get(eventName).forEach(function (value, parent) {
