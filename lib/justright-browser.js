...
var ElementWrapper = exports.ElementWrapper = function () {
    ...
    _createClass(ElementWrapper, [{
        ...
        key: "prependChild",
        value: function prependChild(value) {
            this.getChildElements().prependChild(value);
        }
    }, {
        key: "setChild",
        value: function setChild(value) {
            this.getChildElements().setChild(value);
        }
    }, {
        key: "getFullName",
    ...
    _createClass(InputMapper, [{
        ...
        value: function map(field, destination) {
            ...
            var puller = function puller(event) {
                var fieldDestination = destination; // Determine actual field destination
                var fieldName = inputField.name; // Determine actual field name

                if (inputField instanceof HTMLInputElement) {
                    if (fieldType == "radio") {
                        if (inputField.checked) {
                            fieldDestination[fieldName] = fieldValue;
                        }
                    } else if (fieldType == "checkbox") {
                        if (inputField.checked) {
                            fieldDestination[fieldName] = fieldValue;
                        } else {
                            fieldDestination[fieldName] = null;
                        }
                    } else {
                        fieldDestination[fieldName] = fieldValue;
                    }
                } else if (inputField instanceof HTMLTextAreaElement) {
                    fieldDestination[fieldName] = inputField.value;
                }
            };
            ...
            var pusher = function pusher() {
                var fieldSource = destination; // Determine actual field destination
                var fieldName = inputField.name; // Determine actual field name

                if (inputField instanceof HTMLInputElement) {
                    if (fieldType == "radio" || fieldType == "checkbox") {
                        inputField.checked = fieldSource[fieldName] == fieldValue;
                    } else {
                        inputField.value = fieldSource[fieldName];
                    }
                } else if (inputField instanceof HTMLTextAreaElement) {
                    inputField.value = fieldSource[fieldName];
                }
	...
	_createClass(NodesWrapper, [{
		...
		value: function prependChild(input) {
			if (this._element.firstChild == null) {
				this.add(input);
				return;
			} else if (input instanceof ElementWrapper) {
